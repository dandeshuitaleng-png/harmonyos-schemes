...
> 由 UI UX Pro Max 根据 query `offline tactile workshop accessibility local-first not-surveillance` 生成。 颜色/字体/动效必须再按 `.cursor/skills/harmonyos-scheme-ui/arkui-mapping.md` 映射到 ArkUI 资源， 不得把 CSS、GSAP 或 Web 字体直接进工程。
